...
export function Sidebar() {
  ...
  return (
    <nav className="fixed left-0 top-0 bottom-0 w-56 bg-[var(--c-sidebar-bg)] text-[var(--c-sidebar-text)] flex flex-col">
      {/* Logo */}
      <div className="px-5 py-6 border-b border-[var(--c-sidebar-border)]">
        <h1 className="text-xl font-bold text-white tracking-tight">Canopy</h1>
        <p className="text-xs text-[var(--c-text-tertiary)] mt-0.5">Winsome Purchasing</p>
      </div>
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                isActive
                  ? "bg-[var(--c-sidebar-active)] text-white"
                  : "text-[var(--c-text-tertiary)] hover:bg-[var(--c-sidebar-border)] hover:text-white"
              }`}
            >
              {ICONS[item.icon]}
              {item.label}
